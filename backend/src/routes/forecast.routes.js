...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const forecast = await cacheWrap(`forecast:crop:${cropId}`, () => estimateCropYield(crop, soil, crop.district));
...
router.get('/all', async (req, res) => {
  try {
    const key = `forecast:all:${req.user.role}:${req.user.id}`;
    const cached = await cacheWrap(key, async () => {
      const clauses = [];
      const params = [];
      if (req.user.role === 'FARMER') {
        const ownId = await ownFarmerId(req.user);
